...
- 인증 가드: JWT 없으면 /login 리다이렉트. `lib/api.ts`가 401 수신 시 토큰 폐기 후 /login.
...
- 가로 스크롤(모바일)/전체 표시(데스크톱) 스트립. `position: sticky; top: 헤더높이; backdrop-filter: blur(12px); background: rgba(16,16,19,.82); box-shadow: --shadow-float`.
...
- **선물 항목(ES=F/NQ=F, #1)**: label에 "선물" 배지로 현물과 구분. 값은 현물 마감 대비 등락으로 읽힌다.
...
ECharts gauge(반원). 바늘 = score. 구간 배경 5색: 0-25 `#2E5BFF`, 25-45 `#5B7BD5`, 45-55 `#6B6F7A`, 55-75 `#D5705B`, 75-100 `#F04452`. 하단: label 한글(극단적 공포/공포/중립/탐욕/극단적 탐욕) + F1~F5 미니 바 5개(각 이름·값) + 30일 히스토리 스파크라인.
...
- **배치**: 마지막 실행 상태(stage_status 칩 8개 S1·S1.5·S2·S3·S4·S5·S6·S7), [지금 실행] 버튼(확인 다이얼로그 → POST /batch/run → 진행 폴링). 당일 brief evidence(scope='brief') 다운로드 링크 목록 — market_inputs.json 등(#17).
