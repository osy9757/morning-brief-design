...
- `<AppShell>`: 상단 고정 헤더(로고 "MorningBrief", 종목 검색 인풋 `SearchBox`, 탭 네비: 홈·포트폴리오·설정) + 콘텐츠. 로고는 헤더의 1차 식별 요소로 크게 표시하고, `SearchBox`는 데스크톱/모바일 모두 노출한다. 헤더 아래 **IndexTicker가 sticky로 붙는다**.
- `SearchBox`: 1자 이상 입력 시 약 200ms 디바운스 후 `GET /stocks/search?q={q}` 호출. 입력창 아래 자동완성 드롭다운을 표시하며 항목은 ticker(fw-bold)·name·market/sector 배지로 구성한다. ↑/↓로 하이라이트 이동, Enter 선택 시 `/stocks/{ticker}` 이동, Esc/바깥 클릭/blur로 닫힘. 결과 0건은 "결과 없음" 표시. 홈(`/`)과 종목(`/stocks/*`)이 공개 화면이므로 비로그인에서도 검색과 이동이 동작한다.
...
- 홈 상단 컨트롤: `IndexTicker` 위가 아니라 메인 콘텐츠 최상단에 세그먼트 버튼 `미장 | 국장`을 둔다. 기본값은 미장이다. `미장`은 기존 최상위 brief 필드를, `국장`은 `brief.kr`의 indices/regime/sectors/heatmap/fear_greed/money_flow/recommendations/events/market_wrap을 같은 컴포넌트에 주입한다. `brief.kr`가 null이면 국장 버튼은 비활성화하고 빈 상태를 표시한다.
