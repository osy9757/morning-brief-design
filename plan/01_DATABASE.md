...
CREATE TABLE llm_task_routing (
  task                VARCHAR(40) PRIMARY KEY,  -- 라우팅 대상 task 7종 (profile_test는 라우팅 밖 — 04 §2)
...
CREATE TABLE app_settings (
  ...
  value      TEXT NOT NULL,                     -- llm_engine: claude|codex|manual
  updated_at TIMESTAMPTZ NOT NULL DEFAULT now()
);
-- 시드: ('llm_engine', 'claude') 1행. claude|codex 전역 토글은 7개 LLM task 라우팅을 일괄 갱신하고, manual은 라우팅을 바꾸지 않고 파일 왕복 모드로만 저장한다.
...
CREATE TABLE stock_analyses (                    -- 종목 상세 탭 산출 (요구 7)
  ...
  stock_id BIGINT NOT NULL REFERENCES stocks(id),
  as_of DATE NOT NULL,
  tabs JSONB NOT NULL,                           -- 6키 {fundamental, governance, capability, technical, macro, consensus} 각 {metrics, llm_text}; governance.filing_digest와 fundamental 확장 비율 포함 (#22: overview 탭은 없음)
  ...
  source VARCHAR(12) NOT NULL DEFAULT 'batch',   -- batch|on_demand|manual
...
3. `llm_profiles` 4행 + `llm_task_routing` 7행 — 04 §2 표 그대로 (profile_test는 라우팅 밖, #21).
