...
| 대상 | 소스 | 범위 |
|---|---|---|
| 지수/ETF/종목/선물 일봉 | yfinance `download(tickers, period="2y", auto_adjust=False)` | stocks 테이블 전 종목(선물 ES=F NQ=F YM=F ZQ=F 포함) → prices_daily UPSERT |
| 보유 종목 시간외 최종가 | yfinance `Ticker.info`의 postMarketPrice/preMarketPrice(없으면 null) | 보유 종목만 → S6 after_hours_change_pct (#34) |
| 미국 펀더멘털 | yfinance `Ticker.info` + `quarterly_financials` | 분석 대상(보유+유니버스)만 → financials |
...
| 국내 가격 | pykrx(무키) 우선, Toss API(키 있으면) | KR 종목 → prices_daily |
...
### S2.6 종목 팩터 (분석 대상 = 보유 종목 ∪ 추천 후보 유니버스)
